...
def create_new_window(connection):
    ...
    label_frame = tk.LabelFrame(new_window, text="User Information", padx=10, pady=10, bg="#FFDDC1", fg="black")
    ...
    submit_button = tk.Button(new_window, text="Add New", bg="#A8D5BA", fg="black",
                              command=lambda: submit_data(name_entry, age_entry, gender_var, notes_text, connection))
    submit_button.pack(pady=10)

    update_button = tk.Button(new_window, text="Update", bg="#A8D5BA", fg="black",
    ...
    # Nút xóa
    delete_button = tk.Button(new_window, text="Delete",bg="#f44336", fg="white", font=("Arial", 12), command=lambda: delete_data(connection))
...
def connect_db_window():
    ...
    connection_window.config(bg="#e0f7fa")

    tk.Label(connection_window, text="Database Name:", bg="#e0f7fa").pack(pady=5)
    db_name_entry = tk.Entry(connection_window)
    db_name_entry.pack(pady=5)

    tk.Label(connection_window, text="User:", bg="#e0f7fa").pack(pady=5)
    db_user_entry = tk.Entry(connection_window)
    db_user_entry.pack(pady=5)

    tk.Label(connection_window, text="Password:", bg="#e0f7fa").pack(pady=5)
    db_password_entry = tk.Entry(connection_window, show="*", bg="#fff")
    db_password_entry.pack(pady=5)

    tk.Label(connection_window, text="Host:", bg="#e0f7fa").pack(pady=5)
    db_host_entry = tk.Entry(connection_window)
    db_host_entry.pack(pady=5)

    tk.Label(connection_window, text="Port:", bg="#e0f7fa").pack(pady=5)
    db_port_entry = tk.Entry(connection_window)
    db_port_entry.pack(pady=5)

    connect_button = tk.Button(connection_window, text="Connect", bg="#4caf50", fg="white", command=connect_db)
...
open_connect_window_button = tk.Button(win, text="Connect to Database", command=connect_db_window, bg="#2196f3", fg="white")
